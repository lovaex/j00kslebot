...
var getRandomArbitrary = require('./helpers').getRandomArbitrary;
var tellMeSomething = require('./helpers').tellMeSomething;
var notFoundMessage = require('./helpers').notFoundMessage;
...
var on_message = function (message, rtm, my_id) {
	...
	if (m) {
		...
		client.get_market_by_id(market_id, 
			...
			function(info) {
				text = notFoundMessage(message.user);
				rtm.sendMessage(text, message.channel)
			})
		return
	}
	...
	if (m) {
		...
		client.get_markets_by_query(market_name, function(infos) {
			...
			else text = notFoundMessage(message.user);
			rtm.sendMessage(text, message.channel)
		})
		return;
	}
