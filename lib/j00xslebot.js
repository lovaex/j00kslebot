...
var repeating_callback = function(rtm) 
{
	...
	if (random < 4.0) { // avg number of sponteanous post expected
		...
		var channel_id = startData.channels.filter(function (e) {return e.is_member})[0].id;
		var something = helpers.tellMeSomething();
		rtm.sendMessage(something, channel_id);
	}
}

rtm.on(CLIENT_EVENTS.RTM.RTM_CONNECTION_OPENED, function ()
{
	rtm.ws.my_repeating_callback = setInterval(function () {return repeating_callback(rtm)}, 1000);
});
...
class AIHandler extends ChainOfResponsibilityMessageHandler
{
	...
	handleEvent(slack_event) {
		client.trainAI(slack_event.message);
...
class Greetings extends ChainOfResponsibilityMessageHandler
{
	...
	handleEvent(slack_event) {
		...
		return successor.handleEvent(slack_event);
	}
}
...
var on_message = function (message, rtm, my_id) 
{
	...
	var handler = new ChainOfResponsibilityMessageHandler(
					new AIHandler(
					new ThanksHandler(
					new QueryByMarketId(
					new SearchForMarket(
